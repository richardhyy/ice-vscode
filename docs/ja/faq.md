...
A: フォーキングは会話内に代替パスを作成することができます。メッセージを右クリックして「Fork」を選択すると、その地点から新しいブランチを作成できます。

### Q: モデルはツールやMCPサーバーを使用できますか？
A: はい。メッセージボックスの「Tools」コントロールからツールを有効にします。ICEには組み込みの`fetch_url`ツールが付属し、独自のJavaScriptツールを実行したり、設定で構成したModel Context Protocol（MCP）サーバーに接続したりできます。ツール呼び出しは既定で承認を求めます。詳しくは[カスタムツール](ja/custom-tools.md)を参照してください。
